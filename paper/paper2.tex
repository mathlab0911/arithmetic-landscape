...
$\theta$ at each size. Each evaluation is $O(bN_0)$. Writing $\mathrm{tail}$ for
...
--- \emph{and} (b) \emph{that the envelopes of Proposition~\ref{prop:curv} reach
as far as the Dirichlet neighbourhoods of type} (i), which is
Problem~\ref{prob:annulus}. Fix $d\ge2$ and let $n=\lfloor T/2\rfloor$. With
...
\[
  ...
  \end{array}
\]
with $m_d\le M(18)=0.6005$ for $d=2$. The last row deserves a word. It is
...
$o\bigl((\sqrt3/2)^{b}\bigr)$, and the measured ratios of rows $3$--$5$ to row
...
\item \emph{A gap we have found and not closed}. Lemma~\ref{lem:cover} sorts
      ...
      proof. The natural repair is to widen the certified envelope by applying
      the curvature bound to a sub-product $\{a\in B:a\le A_1\}$, whose zeros are
      spaced $2\pi/A_1$ apart; since $|G_B|$ is at most the sub-product, this
      costs only a smaller $V_0$. We record it as Problem~\ref{prob:annulus}.
...
Extend the certified reach of the envelope of Proposition~\ref{prop:curv} from
$\Theta(\sqrt k)$ standard deviations to the full Dirichlet neighbourhood
$|\theta-2\pi j/q|\le1/(q\tau)$, so that cases~(i) and~(ii) of
Lemma~\ref{lem:cover} meet. Applying the curvature bound to the sub-product over
$\{a\in B:a\le A_1\}$ should suffice: its factors have zeros spaced $2\pi/A_1$
apart, and $|G_B|$ never exceeds it. One must check that the resulting
$V_0(A_1)=\sum_{a\le A_1}a^{2}/4$ is still large enough at the radius required.
\end{problem}
