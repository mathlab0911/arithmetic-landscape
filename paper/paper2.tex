...
must be combined. On the other hand, even $k_0\approx10^{2}$ is far beyond the
range $k\le44$ of the computations in Section~\ref{sec:num}, so
Proposition~\ref{prop:l5a} is genuinely asymptotic and does not certify the
measured values. We state it as an effective bound, not as an explanation of the
data. Note also that $q=4$ and $q=6$ appear in the table only for comparison:
for those two moduli Corollary~\ref{cor:spectrum}(i)--(ii) are exact identities
and no equidistribution input is needed at all.
\end{remark}
...
Corollary~\ref{cor:spectrum} and Proposition~\ref{prop:l5a} it suffices to treat
$q=4$, $q=6$, and a finite list depending on $d$. For the exponential sums that
arise, the closest available technology is that of Dong, Robles, Zaharescu and
Zeindler~\cite{DRZZ25}, who handle the non-principal major arcs for prime
partitions by way of what they call strange and pseudo-differentiable functions.
\end{problem}
