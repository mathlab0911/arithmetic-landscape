...
The canon currently consists of $96$ theorems.
...
\begin{enumerate}
\item Fix $\theta$ with $\theta/2\pi$ having continued-fraction denominator
      $q>Q_0$ (the deep minor arcs; the arcs with $q\le Q_0$ are covered by
      Corollary~\ref{cor:spectrum} and Proposition~\ref{prop:l5a}).
\item Truncate~\eqref{eq:fourier} at $n\le N$ and bound the tail by
      excising a neighbourhood of the zeros of $\cos(a\theta/2)$, controlling
      the excised part by Koksma's inequality against the discrepancy of
      $\{a\theta/2\pi\}_{a\in B}$.
\item Bound each truncated exponential sum by Vinogradov's method, and convert
      the resulting discrepancy bound by Erd\H{o}s--Tur\'an.
\item Optimise the excision parameter at $\delta\asymp1/\log b$, obtaining
      $|G_B(\theta)|\le(1/2+o(1))^{b}$ uniformly on the deep minor arcs.
\end{enumerate}
...
\begin{remark}[Numerical direction check]
For $k=24$ and $k=32$ we sampled $\theta$ whose continued-fraction denominator
exceeds $4\cdot10^{4}$ and computed $|G_B(\theta)|^{1/b}$. The values lie
between $0.41$ and $0.55$, clustering about $0.5$, consistent with the target
$(1/2+o(1))^{b}$. This checks the direction of the estimate, not the estimate.
\end{remark}
