...
Fix $\theta$ whose continued-fraction denominator exceeds $Q_0$; the arcs with
$q\le Q_0$ are covered by Corollary~\ref{cor:spectrum} and
Proposition~\ref{prop:l5a}. Put $\delta_0=(\log b)^{-1}$ and $N_0=b^{1/4}$, and
split $B=B_{\mathrm{good}}\cup B_{\mathrm{bad}}$, where $p$ is bad when
$\|p\theta/2\pi-\tfrac12\|<\delta_0$.

\begin{enumerate}
\item \emph{Harmonics stay minor.} For $n\le N_0$ the reduced denominator of
      $n\theta/2\pi$ is at least $q/\gcd(n,q)\ge b^{1/4}$, so every harmonic
      appearing below lies on a minor arc.
\item \emph{The exponential sums.} With the Dirichlet parameter
      $\tau=b^{1/2}$ the denominators occurring in step~1 satisfy
      $q_n\in[b^{1/4},b^{1/2}]$, and since $N=p_k\asymp k\log k$ this interval
      lies inside $[(\log N)^{A},N(\log N)^{-A}]$ for every fixed $A$ once $k$ is
      large. It is worth recording why this single range suffices. The classical
      treatment splits the circle into three tiers: $q=O(1)$, where the sum is
      genuinely large; $2\le q\le(\log N)^{A}$, where the Siegel--Walfisz theorem
      supplies the saving; and $q>(\log N)^{A}$, where Vaughan's identity does.
      Only the third tier occurs here, because step~1 has already forced
      $q_n\ge b^{1/4}$, which outgrows any fixed power of $\log N$. The first two
      tiers are not lost --- they are precisely the arcs already handled by
      Corollary~\ref{cor:spectrum} and Proposition~\ref{prop:l5a}, where the
      cyclotomic constants $M(q)$ give the bound directly and no exponential sum
      is needed at all. On the surviving range Vaughan's identity gives
      $S_B(\alpha)=\sum_{p\in B}e(p\alpha)\ll
      \bigl(Nq_n^{-1/2}+N^{4/5}+(Nq_n)^{1/2}\bigr)(\log N)^{4}$, hence
      $S_B\ll N(\log N)^{4}b^{-1/8}=o(\pi(N))$; a log-free version with a wider
      range of $q$ is available in~\cite{Sr25}. Summing the head
      $\sum_{n\le N_0}|S_B(n\theta)|/n\le(\log N_0)\max_n|S_B(n\theta)|$ gives
      $o(b)$.
\item \emph{Bad points.} Each bad $p$ contributes at most $\log2$
      to~\eqref{eq:fourier}, so it suffices that $|B_{\mathrm{bad}}|=o(b)$. This
      needs no input beyond step~2. Majorising the indicator of the interval
      $\|x-\tfrac12\|<\delta_0$ by a Selberg polynomial of degree $N_0$ --- the
      one-interval form of the Erd\H{o}s--Tur\'an inequality --- gives
      \[
        |B_{\mathrm{bad}}|\;\le\;b\Bigl(2\delta_0+\frac{1}{N_0}\Bigr)
          +\sum_{n\le N_0}c_n\,\bigl|S_B(n\theta)\bigr|,
        \qquad |c_n|\le\min\Bigl(2\delta_0+\tfrac{1}{N_0},\tfrac1n\Bigr),
      \]
...
\item \emph{Good points.} For $p$ good the series~\eqref{eq:fourier} is
      ...
      $C/(N_0\delta_0)=C\log b\cdot b^{-1/4}=o(1)$.
\end{enumerate}
...
The parameters $\delta_0=1/\log b$ and $N_0=b^{1/4}$ are meaningless at $b=23$,
so we tested the steps above at $b$ up to $1999$ (each evaluation is $O(b)$).
On deep minor arcs:
\[
  \begin{array}{c|cccccc}
    b & 23 & 31 & 99 & 299 & 999 & 1999\\\hline
    |B_{\mathrm{bad}}|/b & 0.565 & 0.581 & 0.455 & 0.341 & 0.288 & 0.253\\
    D_B+2\delta_0 & 0.795 & 0.712 & 0.503 & 0.390 & 0.304 & 0.282\\
    N_0^{-1}\!\sum_{n\le N_0}|S_B(n\theta)|/n\ \text{(per element)}
      & 0.239 & 0.145 & 0.085 & 0.085 & 0.044 & 0.039
  \end{array}
\]
The bound of step~3 holds in every case, and the head of step~2 decreases. The
quantity $-b^{-1}\log|G_B(\theta)|-\log2$, which the argument asserts is $o(1)$,
has magnitude at most $0.123$ at $b=99$ and at most $0.024$ at $b=1999$ over six
sampled $\theta$ per size.

Two honest caveats. First, the tail bound of step~4 is $C\log b\cdot b^{-1/4}$,
which does not fall below $1$ until $b$ is of order $10^{6}$; it is $o(1)$, but
slowly. Second --- and this is why that does not matter --- the conclusion needs
only $\tfrac12e^{o(1)}<\sqrt3/2$, i.e.\ the $o(1)$ above to stay below
$\log\sqrt3=0.549$. The measured values are an order of magnitude smaller than
that already at $b=99$.
\end{remark}
...
\emph{Assume the deep-minor bound of Section~\ref{sec:l5c}}, i.e.\ that
$|G_B(\theta)|\le(1/2+o(1))^{b}$ off all the envelopes of
Proposition~\ref{prop:curv}; that section establishes it at the level of a
sketch. Fix $d\ge2$ and let $n=\lfloor T/2\rfloor$. With $B=B_d(k)$, $b=|B|$, as
$k\to\infty$,
\[
...
not proved, and we prefer to carry it explicitly rather than to bury it. It
...
$|G_B|\le(1/2+o(1))^{b}$.
\emph{(6)} Integrating over each region and dividing by $\Main$, every
contribution other than $q=6$ and $q=4$ is smaller by a factor
$((m_d+o(1))/(\sqrt3/2))^{b}$; taking the maximum and minimum over the bounded
window gives the displayed formula.
\end{proof}
...
\item \emph{Proved on paper}: Proposition~\ref{thm:cyc} and
      ...
      Proposition~\ref{prop:curv}, Theorem~\ref{thm:p5exact}.
