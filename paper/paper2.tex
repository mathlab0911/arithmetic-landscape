...
\item \emph{The exponential sums.} With the Dirichlet parameter
      ...
      large. On that range Vaughan's identity gives
      ...
      range of $q$ is available in~\cite{L25}. Summing the head
      $\sum_{n\le N_0}|S_B(n\theta)|/n\le(\log N_0)\max_n|S_B(n\theta)|$ gives
      $o(b)$.
\item \emph{Bad points.} $|B_{\mathrm{bad}}|\le(D_B(\theta)+2\delta_0)b=o(b)$,
      and each bad $p$ contributes at most $\log2$ to~\eqref{eq:fourier}, so
      their total effect is $o(b)$.
...
One can instead avoid~\eqref{eq:fourier} and argue through the discrepancy of
$\{p\theta/2\pi\}_{p\in B}$ directly: bound it by Erd\H{o}s--Tur\'an from the
same exponential sums, then apply Koksma's inequality to the truncated function
$\max(\log|\cos\pi x|,\log\delta)$. The two routes give the same conclusion; we
prefer the one above because the decomposition~\eqref{eq:fourier} is exact.
\end{remark}
...
\begin{theorem}[Asymptotic flatness; scope as in Section~\ref{sec:remains}]\label{thm:t2main}
Fix $d\ge2$ and let $n=\lfloor T/2\rfloor$. With $B=B_d(k)$, $b=|B|$, as
$k\to\infty$,
\[
...
$10^{4}$ samples each; it held in every case, and in fact the sharper bound
$(e\delta/2J)^{J}$ also held throughout. We state only the weaker form because
that is what the argument above proves; the conclusion is unaffected, since $J$
is chosen after $\delta$.
\end{remark}
