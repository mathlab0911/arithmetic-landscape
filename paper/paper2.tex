...
\item \emph{The exponential sums.} Vaughan's identity bounds
      $S_B(\alpha)=\sum_{p\in B}e(p\alpha)$ by
      $\ll N(\log N)^{3}q_n^{-1/2}$ on such arcs; summing the head
...
\begin{proposition}[Sketch]\label{prop:p5tail}
Fix $J$ and $\delta>0$. Then
$\mathbb{P}\bigl[a_{(J)}\le\delta N/k\bigr]\le(e\delta/2J)^{J}$, whence
$\Gamma(R_k)\ge2^{-J}\delta N/k$ with probability at least
$1-(e\delta/2J)^{J}$. Choosing $\delta=1/2$ and then $J$ large gives
$\Gamma(R_k)\ge c_0\log k$ with probability $1-o(1)$, and therefore
$\Gamma(P_k)/\Gamma(R_k)\to0$ in probability.
\end{proposition}

\noindent
The tail bound was checked over $48$ combinations
$k\in\{100,400,1600,3200\}$, $J\in\{1,2,4,8\}$, $\delta\in\{0.25,0.5,1\}$ with
$10^{4}$ samples each; it held in every case, with the measured probability
between one half and one seventh of the bound. A complete proof will appear with
the assembly of Problem~\ref{prob:assembly}.
