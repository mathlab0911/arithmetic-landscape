...
% r120: paper 1 had neither of these while papers 2-4 did, which is the mechanical reason its
% status notes lived in prose (F56).  Added with the definition change.
\newcommand{\STATUS}[1]{\textcolor{blue}{\textsf{[#1]}}}
\newcommand{\TODO}[1]{\textcolor{red}{[\textbf{TODO} #1]}}

\title{Arithmetic landscapes I:\\
the gap series}
...
\begin{abstract}
Let $A$ be a finite set of odd positive integers, $\sigma$ the sum, and
\[
  r_A(n)=\#\{S\subseteq A:\sigma(S)=n\}
\]
the number of representations of $n$ as a subset sum of $A$. For $d\ge0$ split
$A$ at $2d$: let $I_d=\{a\in A:a\le2d\}$ be the small elements and
$B_d=A\setminus I_d$ the rest. This paper is about the quantity
\[
  \lm_A(n)\;=\;r_A(n)\;+\;\sum_{d\ge1}
    \Bigl(r_{B_d}(n+d)+r_{B_d}\bigl(n-d-\sigma(I_d)\bigr)\Bigr),
\]
a weighted count of representations by the \emph{truncations} of $A$. The sum is
finite: every term vanishes once $2d\ge\max A$.

We show that the ratio $\lm_A(n)/r_A(n)$ converges, \emph{for every target $n$},
to a quantity that can be read off $A$ by counting, with no subset sum computed:
writing $M=\max A$ and $N_A(d)=\#\{a\in A:a\le2d\}$,
\[
  \Gamma(A)\;=\;1+2\sum_{d=1}^{(M-1)/2}2^{-N_A(d)}\;\in\;\mathbb{Q},
\]
equivalently a dyadically weighted sum of the gaps of $A$, dominated by its
smallest elements. For the odd primes $\Gamma=5.3492879\ldots$, and the small
initial gaps --- the twin pairs --- are what make it small.

The quantity $\lm_A(n)$ has a second description, which is where the question
came from and which the notation records. Give the subsets $S$ the energy
$|\sigma(S)-n|$ and call two subsets adjacent when they differ in one element;
then $\lm_A(n)$ counts the $S$ from which every single-element change increases
the energy, that is, the strict local minima of the subset-sum landscape, and
$r_A(n)$ counts its ground states. That the two descriptions agree is an
identity between integers, not an approximation, and it is one of the theorems
below. A reader who wants no landscape may take the displayed formula as the
definition and read the rest as a statement about representation counts of
truncations.

We prove a classification theorem for those local minima; a window
identity $W_D(A)=\Gamma(A)+(2D+1-M)2^{-k}$ exhibiting $\Gamma$ as the total
measure of the windows in which a configuration can be a local minimum; sharp
bounds $3-2^{1-(M-1)/2}\le\Gamma(A)\le M$, with equality on the left exactly for
$A=\{1,3,\dots,M\}$ and on the right exactly for $A=\{M\}$; and an exact
stratification --- the identity displayed above, proved as an equality of
integers. If the representation counts $r_{B_d}$ are \emph{flat} to within a
factor $1+\varepsilon$ on an explicit finite window, then
\[
\frac{W_D(A)}{1+\varepsilon}\;\le\;\frac{\lm_A(n)}{r_A(n)}\;\le\;(1+\varepsilon)\,W_D(A).
\]
...
$(\lm/r)/\Gamma\to1$ with standard deviation $0.0067$ at $k=20$ and
...
Every theorem in the structural part of the paper --- the classification, the
window identity, the exact stratification and the sandwich, together with the
lemmas they rest on --- is formally verified in Lean~4 with Mathlib, as are the
character computations behind the modulus-$4$ obstruction; no proof contains
\Lean{sorry} and no additional axiom is used. One statement is verified only in
part: for the extremality of the modulus $6$ the finite check is formalised,
while the supremum over all $q$ rests on the classical values of $\Phi_q(-1)$.
Each formal statement is named in the text and indexed in the appendix. The
development is public at
...
Throughout, $A=\{a_1<\dots<a_k\}$ is a finite set of odd positive integers. Write
$M=a_k$ for its largest element and
\[
  N_A(d)\;=\;\#\{i:a_i\le 2d\},\qquad d\ge1,
\]
for its counting function: the number of elements at or below $2d$. Since the
elements are distinct and odd, $N_A(d)\le d$ for every $d$, with equality
exactly when $A$ contains every odd number below $2d$. This one inequality is
responsible for most of what follows.

\begin{definition}[gap series]\label{def:gap}
The \emph{gap series} of $A$ is
\[
  \Gamma(A)\;=\;1+2\sum_{d=1}^{(M-1)/2}2^{-N_A(d)}\;\in\;\mathbb{Q}.
\]
\end{definition}

The sum stops at the last index for which the layer $\{a\in A:a>2d\}$ is
non-empty; beyond it every term is $2^{-k}$ and carries no information about
$A$. Two rewritings say what the definition is.

\begin{proposition}[gap form]\label{prop:gapform}
$\displaystyle\Gamma(A)=a_1+\sum_{j=1}^{k-1}(a_{j+1}-a_j)\,2^{-j}.$
\end{proposition}

\begin{proof}
$N_A(d)=j$ exactly for $(a_j+1)/2\le d\le(a_{j+1}-1)/2$, which is
$(a_{j+1}-a_j)/2$ values of $d$, and $N_A(d)=0$ for $d\le(a_1-1)/2$, which is
$(a_1-1)/2$ values. Collecting equal terms,
$\Gamma(A)=1+(a_1-1)+\sum_{j=1}^{k-1}(a_{j+1}-a_j)2^{-j}$.
\end{proof}

\begin{proposition}[$\Gamma$ is the value at $1$ of a generating function]\label{prop:gengamma}
\STATUS{proved; the identity is Proposition~\ref{prop:gapform} regrouped, and it is checked
numerically to $10^{-30}$ at twelve points on four profiles in Part~III's
\texttt{leeyang5\_r183}.}
Write $m_0=(a_1-1)/2$ and $m_j=(a_{j+1}-a_j)/2$ for $1\le j\le k-1$ --- the number of $d$ at
which $N_A(d)=j$ --- and set
\[
  G_A(z)\;=\;\sum_{j\ge0}m_j\,2^{-j}z^{\,j}.
\]
Then
\[
  \Gamma(A)\;=\;1+2\,G_A(1).
...
\begin{proof}
Each $d$ with $N_A(d)=j$ contributes $2^{-j}$ to the sum of
Definition~\ref{def:gap}, and there are $m_j$ of them. \qed
\end{proof}

\begin{remark}[why this is stated separately, and what it buys]\label{rem:gengamma}
The identity is trivial and the observation is not: \textbf{$\Gamma$ is a number we had only
ever evaluated at one point of a function we had not noticed we had.} Two consequences follow at
once, and a third is developed in Part~III.

\emph{First, a dichotomy that the value at $1$ cannot see.} Let $R$ be the radius of convergence
of $G_A$ in the limit $k\to\infty$. Since $G_A(1)=(\Gamma(A)-1)/2$, the statement $R\ge1$ is
\emph{exactly} the statement that $\Gamma$ is finite --- while $R>1$ is the strictly stronger
statement that the layer weights $m_j2^{-j}$ decay \emph{geometrically}. The two are
indistinguishable at $q=\tfrac12$, where both say only that a sum converges.

\emph{Second, the odd numbers in closed form.} There $m_j=1$ for $1\le j\le k-1$ and $m_0=0$, so
$G_A(z)=\sum_{j\ge1}(z/2)^{j}=z/(2-z)$ for $|z|<2$, and $R=2$.

\emph{Third, in Part~III.} Deforming the measure replaces the evaluation point: the biased
invariant is $1+G_A(2q)+G_A(2-2q)$, so the whole one-parameter family is $G_A$ read at two
places instead of one, and the analytic structure of $G_A$ --- its radius, its behaviour on the
circle --- becomes a statement about the landscape. \emph{A quantity evaluated at a single point
is a function one has not noticed one has}, and this proposition is where that is recorded.
\STATUS{the identity is proved; the consequences drawn here are proved; the use made of them for
the deformed measure is Part~III's and carries its own statuses.}
\end{remark}

\begin{proposition}[$\Gamma$ is Schur-concave]\label{prop:schur}
\STATUS{proved; the proof is Abel summation followed by the Schur--Ostrowski criterion.
Measured for confirmation only: over $4000$ majorization-comparable pairs of sets of distinct
odd integers, $\Gamma$ was larger on the more equal set in every case that was not a tie;
\texttt{schur\_r149}.}
Regard $\Gamma$ as a function of the multiset. If $A$ majorizes $B$ --- same sum, $A$ the more
spread out --- then $\Gamma(A)\le\Gamma(B)$.
\end{proposition}

\begin{proof}
Abel summation applied to Proposition~\ref{prop:gapform} gives
$\Gamma(A)=\sum_{j=1}^{k}w_ja_j$ with $a_1<\dots<a_k$ and
\[
  (w_1,\dots,w_k)=\bigl(\tfrac12,\tfrac14,\dots,2^{-(k-1)},2^{-(k-1)}\bigr),
\]
positive and \emph{non-increasing}. So $\Gamma$ is an ordered weighted average taken along the
ascending order. It is symmetric as a function of the multiset, and on the open region where
the entries are distinct $\partial\Gamma/\partial a_i=w_i$; if $a_i>a_j$ then $i>j$, hence
$w_i\le w_j$ and $(a_i-a_j)(\partial_i\Gamma-\partial_j\Gamma)\le0$. That is the
Schur--Ostrowski condition for Schur-concavity, and $\Gamma$ is continuous, so it holds on the
closure.\qed
\end{proof}

\begin{remark}[what Schur-concavity buys, and what it explains]\label{rem:schur}
Two things, one technical and one conceptual.

\emph{Technical.} $\Gamma$ now sits inside the Hardy--Littlewood--P\'olya framework, so
Karamata's inequality and the standard majorization toolkit apply to it without further work.
Any statement of the form ``$\Gamma$ is extremal on the most/least spread set with a given sum''
is a corollary rather than a computation.

\emph{Conceptual.} Schur-concavity says exactly that \emph{$\Gamma$ rewards equality and
punishes spread}. That is the precise version of the informal description used throughout this
series --- that $\Gamma$ is a \emph{bottom} statistic, decided by the smallest elements --- and
it is why $\Gamma$ separates sets that a second moment cannot: variance is Schur-\emph{convex},
so it moves the other way. The two are not competing measures of the same thing; they have
opposite monotonicity under the same partial order.
\STATUS{derived from Proposition~\ref{prop:schur}; the remark about variance is the standard
fact that $\sum(a_i-\bar a)^2$ is Schur-convex.}
\end{remark}

\begin{proposition}[enumeration form]\label{prop:enumform}
$\displaystyle\Gamma(A)=\sum_{j=1}^{k}\frac{a_j}{2^{j}}+\frac{a_k}{2^{k}}.$
\end{proposition}

\begin{proof}
Abel summation applied to Proposition~\ref{prop:gapform}.
\end{proof}

\STATUS{Proposition~\ref{prop:enumform} is Theorem~\ref{thm:window} at
$D=(M-1)/2$, and is Lean-verified in that form
(\Lean{windowSeries\_eq\_gapSeries}); Proposition~\ref{prop:gapform} then follows
from it by the same summation by parts. The bounds of \S\ref{sec:extremal} are
verified as well.}

Three remarks fix the character of the quantity.

First, \textbf{$\Gamma$ is an invariant of the set.} Definition~\ref{def:gap} never
mentions an ordering: $N_A$ counts. This is what one wants of a quantity that
is to describe the subset-sum landscape of $A$, since that landscape depends on
the multiset alone. It also distinguishes $\Gamma$ from the statistics
ordinarily attached to a set of weights --- mean, variance, coefficient of
variation --- not by being order-sensitive, but by being sensitive to
\emph{where the elements sit} rather than to their spread.

Second, the name is now exact: by Proposition~\ref{prop:gapform}, $\Gamma$ is a
dyadically weighted sum of the \emph{gaps} of $A$, dominated by the earliest
ones. For the odd primes $3,5,7,11,\dots$ the small initial gaps --- in
particular the twin pairs $(3,5)$, $(5,7)$, $(11,13)$, $(17,19)$ --- force
$\Gamma$ to be small: $\Gamma(3,5,\dots,73)=1402281/262144\approx5.34928$,
against $\approx8.09$ on average for a random odd sequence of the same length
drawn from the same range.

Third, by Proposition~\ref{prop:enumform} the enumeration series
$\sum_ja_j2^{-j}$ --- which does depend on an ordering, and which is how this
invariant was first written --- agrees with $\Gamma$ up to $a_k2^{-k}$, so the
two have the same limit along any sequence with $a_k2^{-k}\to0$. The choice
between them is therefore a choice of finite representative, and
Definition~\ref{def:gap} is the better one twice over: it is manifestly a set
invariant, and it is the closer approximant --- for the odd primes at $k=20$ it
lies $1.04\cdot10^{-5}$ from the limit against $8.01\cdot10^{-5}$ for the
enumeration form.\footnote{An earlier circulated version of this paper defined
$\Gamma$ by the enumeration series. The two differ by $a_k2^{-k}$ and have the
same limit; every asymptotic statement in this series of papers is unaffected.
That version also carried the title \emph{The gap series of an integer sequence};
the present title places it in the series, and no content was removed in the
change.}
...
construction and ours share no content beyond the word ``gap''. Nor is it related
to the \emph{gap series} of complex analysis --- a power series whose exponents
have Hadamard gaps, also called a lacunary series --- where the gaps are in the
exponents rather than in the coefficients, and the object of study is
continuation across the circle of convergence.
...
\subsection{Terminology, in one table}\label{sec:terminology}

This programme names a few objects that additive number theory has no name for,
and borrows a few names from statistical physics for objects it does. A reader
who meets one of them cold can look it up here; each is defined again where it
is used.
...
\toprule
term & what it denotes, in ordinary language\\
\midrule
representation count $r_A(n)$ & $\#\{S\subseteq A:\sigma(S)=n\}$. Standard. Every count
of this kind in this series carries the letter $r$.\\[3pt]
truncation $B_d$ & $\{a\in A:a>2d\}$, the set with its elements up to $2d$ removed;
$I_d$ is the removed part.\\[3pt]
\emph{gap series} $\Gamma(A)$ & the rational number
$1+2\sum_{d}2^{-\#\{a\in A:\,a\le2d\}}$, read off $A$ by counting. Equivalently a
dyadically weighted sum of the gaps of $A$; hence the name.\\[3pt]
\emph{subset-sum landscape} & the set of subsets of $A$ with the function
$S\mapsto|\sigma(S)-n|$ and single-element changes as moves. A way of speaking, not an
object: every statement about it in this paper is restated as a statement about
representation counts.\\[3pt]
energy, ground state & $|\sigma(S)-n|$, and a subset with energy $0$. Borrowed from
statistical physics, where this landscape is the number partitioning problem.\\[3pt]
\emph{valley}, $\lm_A(n)$ & a subset from which every single-element change increases
the energy, and the number of them. Equal, as an identity between integers, to a
weighted sum of representation counts of the truncations
(Theorem~\ref{thm:stratification}).\\[3pt]
\emph{window series} $W_D(A)$ & $1+2\sum_{d\le D}2^{-\#\{a\in A:\,a\le2d\}}$, the same sum
stopped at $D$. Equals $\Gamma$ at the last informative $D$. The
\emph{window identity} is that equality, Theorem~\ref{thm:window}; earlier versions also
called $W_D$ the \emph{window measure}, which is the same object.\\[3pt]
\emph{stratum} & the set of valleys at a fixed distance $d$ from the target. The
\emph{stratification} is the decomposition of $\lm_A(n)$ into strata.\\[3pt]
\emph{flatness} $\varepsilon_d$ & the relative oscillation
$\max|r_{B_d}(m)/r_{B_d}(m')-1|$ of a representation count over a bounded window of
targets. Small $\varepsilon_d$ is what makes the strata comparable.\\[3pt]
\emph{ripple} & the periodic oscillation that a sub-peak of $|F_A|$ contributes to the
representation counts as the target varies. Its size is what $\varepsilon_d$ measures;
Part~II computes it for the primes.\\[3pt]
the \emph{sandwich} & the two-sided bound of Theorem~\ref{thm:sandwich}: if
$\varepsilon_d$ is small then $\lm_A(n)/r_A(n)$ lies within a factor $1+\varepsilon$ of
$W_D(A)$.\\
\bottomrule
\end{tabular}
\end{table}
...
\noindent
Two of these are the point of the paper and the rest are scaffolding. The gap
series is a new object; the identity between the valley count and the
representation counts of the truncations is a new theorem. Everything else is
vocabulary for stating them.
...
\[
  \lm_A(n)\;=\;r_A(n)+\sum_{d\ge1}\bigl(\ovr_A(n,d)+\undr_A(n,d)\bigr),
\]
...
$r_A(n)=\sum_{J\subseteq I_d}r_{B_d}(n-\sigma(J))$.
\end{theoremC}
...
Combining Theorems~B and~D, $\lm/r$ is pinned to $\Gamma(A)$ with a
...
$\lm/r$ to the flatness of truncated representation counts, are new. That
sentence is a literature pass and not a proof of novelty: it records the absence
of evidence over the searches we made, and the searches we made are ordinary
ones. \STATUS{literature pass, not a proof of novelty: absence of evidence over
the searches made, at a database of the size and date stated}
The sequence searches were made and are recorded. On 2026-08-11, against a
database of $398\,208$ sequences, none of four queries to the OEIS returned a
match: the numerators $3,11,29,69,151,\dots$ of $2^{k}\Gamma(P_k)$; the digits
of $\Gamma(P)$; the number of strict local minima of the first $k$ odd primes at
the central target; and the number of ground states of the same instances. A
positive control run first --- the decimal digits of $\sqrt2$ --- returned
\texttt{A002193} at the head of its results, so the route does find a sequence
that is there. The full record, with the exact query strings and the database
timestamp, is \texttt{lean/pnp/oeis\_r119.log}. The database grows; if $\Gamma$
or any of these is catalogued now, we would be glad to be told.
...
and we reproduce it (\S\ref{sec:cv}). But $CV$ is a global second moment,
whereas Theorem~B shows it is $\Gamma$ --- a dyadically weighted sum of gaps,
dominated by the smallest elements --- that equals the window series $W_D$.
The two can be decoupled, and when they are, the landscape follows $\Gamma$:
pairs of sets with \emph{exactly} equal $CV$ but $\Gamma$ a factor $2.1$ apart
have $\lm/r$ a factor $2.1$ apart, while pairs with exactly equal $\Gamma$
and $CV$ a factor $1.6$ apart have $\lm/r$ within $8\%$
(Table~\ref{tab:twins}). In this precise sense $\Gamma$ subsumes $CV$.
...
\[
  ...
  r_A(n)=\#\{S:\sigma(S)=n\},\quad
  \lm_A(n)=\#\{S:S\text{ strict local min}\}.
\]
The middle one is the representation count of $n$ as a subset sum of $A$; in the
landscape reading it is the number of ground states, and earlier versions of this
paper wrote it $\deg_A(n)$ for that reason. We use $r_A$ throughout, so that the
same kind of object --- a representation count --- always carries the same symbol,
whether the set is $A$ or one of its truncations $B_d$.
...
representable'', and $r_A(n)$ is the number of representations, denoted
$r_A(n)$ below.
...
$W_D$ is the annealed prediction for $\lm_A(n)/r_A(n)$. Sections
...
for all $i$. Then
\[
  W_D(A)\;=\;\sum_{j=1}^{k}\frac{a_j}{2^{j}}\;+\;\frac{2D+1}{2^{k}}
        \;=\;\Gamma(A)+\frac{2D+1-a_k}{2^{k}}.
\]
In particular $W_D(A)=\Gamma(A)$ at the smallest admissible window,
$D=(a_k-1)/2$: \emph{the gap series is the window series evaluated at the last
non-empty layer}, and Definition~\ref{def:gap} is that statement taken as the
definition. Enlarging $D$ beyond $(a_k-1)/2$ adds empty layers, each
contributing exactly $2\cdot2^{-k}$, which is the $2D+1-a_k$ above.
\textup{(Lean: \Lean{windowSeries\_eq\_gapSeries}, in the first form.)}
\end{theorem}
...
\subsection{Why the enumeration form needs the order}\label{sec:order-matters}

Both sides of Theorem~\ref{thm:window} as stated in the second form are
invariants of the set: $W_D$ counts, and so does $\Gamma$. The
\emph{enumeration} form $\sum_ja_j2^{-j}$ does not, and the hypothesis that $A$
be non-decreasing is what the first form of the theorem pays for it. The
failure is worth exhibiting, because it is also the reason
Definition~\ref{def:gap} was chosen over the enumeration series.

\begin{example}\label{ex:order}
Take the sequence $(5,3)$ and $D=2$. The counting function does not see the
order: $N(1)=0$, $N(2)=1$, so $W_2=1+2(1+\tfrac12)=4$, and the gap series of the
underlying set $\{3,5\}$ is $\Gamma=1+2(1+\tfrac12)=4$ as well --- here
$D=(M-1)/2$, so the two agree exactly. The enumeration series does see the
order: read in the given order it is
$\tfrac52+\tfrac34=\tfrac{13}4$, and $\tfrac{13}4+\tfrac54=\tfrac92\ne4$; read
in increasing order it is $\tfrac32+\tfrac54=\tfrac{11}4$, and
$\tfrac{11}4+\tfrac54=4$. Both computations are \Lean{\#eval}-checked in the
Lean development.
\end{example}

Two consequences. Inside this paper: the sorting hypothesis in
Theorem~\ref{thm:window} is not removable, so any argument that reaches for the
enumeration form must carry it. Outside: no order-blind \emph{spread}
statistic can play the role of $\Gamma$ either, which is the subject of
\S\ref{sec:cv}. Being an invariant of the set is necessary, and
Example~\ref{ex:order} shows the enumeration series fails it; \S\ref{sec:cv}
shows the standard invariants that pass it fail for a different reason.

\section{Extremal values}\label{sec:extremal}

Definition~\ref{def:gap} makes the range of $\Gamma$ visible at once. Fix the
largest element $M$ and write $D=(M-1)/2$, so that the sum in
Definition~\ref{def:gap} runs over $1\le d\le D$. Every term $2^{-N_A(d)}$ lies
between $2^{-d}$ and $1$, the upper bound because $N_A(d)\ge0$ and the lower
because $N_A(d)\le d$ --- there are only $d$ odd numbers at or below $2d$. The
two extremes are attained by the two extreme sets.

\begin{theorem}[extremal values]\label{thm:extremal}
Let $A$ be a finite set of odd positive integers with largest element $M$, and
put $D=(M-1)/2$. Then
\[
  3-2^{\,1-D}\;\le\;\Gamma(A)\;\le\;M .
\]
The upper bound is attained if and only if $A=\{M\}$, and the lower bound if and
only if $A=\{1,3,\dots,M\}$.
\textup{(Lean: \Lean{windowSeries\_bounds}, \Lean{windowSeries\_eq\_max\_iff},
\Lean{windowSeries\_min\_iff\_all\_odds}.)}
\end{theorem}

\begin{proof}
Since $N_A(d)\ge0$ we have $2^{-N_A(d)}\le1$, so
$\Gamma(A)\le1+2D=M$; and equality forces $N_A(d)=0$ for every $d\le D$, that
is, $A$ has no element at or below $2D=M-1$, that is, $A=\{M\}$. Conversely
$\Gamma(\{M\})=1+2D=M$.

For the lower bound, the odd numbers at or below $2d$ are exactly
$1,3,\dots,2d-1$, so $N_A(d)\le d$ and $2^{-N_A(d)}\ge2^{-d}$. Summing the
geometric series,
\[
  \Gamma(A)\;\ge\;1+2\sum_{d=1}^{D}2^{-d}\;=\;1+2\bigl(1-2^{-D}\bigr)\;=\;3-2^{\,1-D}.
\]
Equality forces $N_A(d)=d$ for every $d\le D$; but $N_A(D)=D$ already suffices,
since $N_A(D)=D$ says that $A$ contains all $D$ of the odd numbers
$1,3,\dots,2D-1=M-2$, and then $N_A(d)=d$ holds for every $d\le D$
automatically. Adjoining $M=\max A$ gives $A\supseteq\{1,3,\dots,M\}$, and $A$
contains nothing else, being a set of odd numbers bounded by $M$. Conversely
$N_{\{1,3,\dots,M\}}(d)=d$ for $d\le D$, which gives equality.
\end{proof}

\begin{remark}
The lower bound is attained as soon as it is attained in the last layer: a
single equation, $N_A(D)=D$, pins $A$ completely. Nothing similar is true of
the upper bound, where all $D$ equations $N_A(d)=0$ are needed --- but they are
all implied by the last one, $N_A(D)=0$, for the same reason of monotonicity.
Both extremal characterisations are therefore statements about the top layer
alone.
\end{remark}

The mechanism behind the theorem is monotonicity, which is worth isolating,
since it is what licenses reading $\Gamma$ as a measure of sparsity.

\begin{corollary}[strict monotonicity]\label{cor:monotone}
Let $A$ be a finite set of odd positive integers with largest element $M$, and
let $b$ be odd with $b\notin A$ and $b<M$. Then
\[
...
\begin{proof}
Adjoining $b$ does not change $M$, so the two sums run over the same range
$1\le d\le D$. Termwise,
$N_{A\cup\{b\}}(d)=N_A(d)+\mathbf 1[\,b\le2d\,]\ge N_A(d)$, so every term
weakly decreases. Since $b$ and $M$ are odd and $b<M$ we have $b\le M-2=2D-1$,
so the indicator is $1$ at $d=D$ and that term strictly decreases.
\end{proof}

Thus $\Gamma$ decreases strictly as $A$ fills in, from $M$ at the sparsest set
$\{M\}$ down to $3-2^{1-D}$ at the densest set $\{1,3,\dots,M\}$; and as
$M\to\infty$ the lower bound tends to $3$, so
\[
  \Gamma(\{1,3,5,\dots\})\;=\;3
\]
in the sense that the gap series of the first $D$ odd numbers converges to $3$,
the odd numbers being the unique minimiser at every finite stage. Against this
scale, $\Gamma(P_{20})=1402281/262144\approx5.349$ for the first twenty odd
primes lies near the bottom of the interval $[3-2^{-35},73]$ available to a set
with $M=73$: on the logarithmic scale of that interval the primes sit at
$18\%$. This is the sense in which the primes are a \emph{dense} set of weights,
and the reason their subset-sum landscapes are comparatively smooth.

\begin{remark}[on the two forms]\label{rem:convergence}
Definition~\ref{def:gap} and the enumeration form
$\sum_ja_j2^{-j}$ of Proposition~\ref{prop:enumform} differ by $a_k2^{-k}$ and
converge to the same limit whenever $a_k2^{-k}\to0$, which holds for the primes
($a_k\sim k\log k$) and for any sequence of subexponential growth. At finite
$k$ they differ, and Definition~\ref{def:gap} is the closer approximant: for the
odd primes at $k=20$ the layer form lies $1.04\cdot10^{-5}$ from the limit
against $8.01\cdot10^{-5}$ for the enumeration form, a factor of $7.7$. The
reason is visible in Theorem~\ref{thm:window}: the enumeration form is the
window series with the whole tail $(2D+1)2^{-k}$ still attached, whereas
Definition~\ref{def:gap} is the window series at the last informative layer,
with the tail removed exactly.
\end{remark}
...
$\ovr_A(n,0)=\undr_A(n,0)=r_A(n)$. If $a_i\le2d$ for all $i$ and $d\ge1$,
...
\[
  \lm_A(n)\;=\;r_A(n)\;+\;\sum_{d=1}^{D}\bigl(\ovr_A(n,d)+\undr_A(n,d)\bigr).
\]
...
$r_A(n)=\sum_{J\subseteq I}r_{A|_{I^{c}}}(n-\sigma(J))$ for any $I$.
...
\subsection{What has been achieved}\label{sec:achieved}
...
Suppose $r_A(n)>0$ and $A$ is $\varepsilon$-flat at $(n,d)$ for every
$1\le d\le D$. Then
\[
  \frac{W_D(A)}{1+\varepsilon}\;\le\;\frac{\lm_A(n)}{r_A(n)}\;\le\;(1+\varepsilon)\,W_D(A),
\]
...
\begin{proof}
Fix $d$. By Corollary~\ref{cor:deg-rep} the denominator $r_A(n)$ is the sum
...
\[
  \frac{2^{-N(d)}}{1+\varepsilon}\;\le\;\frac{\ovr_A(n,d)}{r_A(n)}\;\le\;(1+\varepsilon)\,2^{-N(d)},
\]
and the same for $\undr$. Summing over $1\le d\le D$ and adding the $d=0$ term
$r/r=1$, then invoking Theorem~\ref{thm:stratification}, gives the result;
...
\Lean{max\_abs\_cos\_sin\_sq}, \Lean{max\_abs\_cos\_sin\_quarter\_even} and
\Lean{max\_abs\_cos\_sin\_quarter\_odd} give the parity dichotomy in (ii), the
spread being $2^{b/2}\max(|\cos\varphi|,|\sin\varphi|)$ with
$\varphi=\pi(p-q)/4$ and $p-q\equiv b\pmod 2$.)}
\end{theorem}
...
Proposition~\ref{prop:mod6}. One cannot, however, upgrade the first half of that
sentence to a theorem covering all odd sequences: no window analogue of
Theorem~\ref{thm:floor} holds, and Remark~\ref{rem:no-window-floor} exhibits
sequences with $\varepsilon_2=0$ exactly.
...
$n=\lfloor T/2\rfloor$. The fifth column is the window series $W_D$ at the last
informative layer $D=(a_k-1)/2$, which by Theorem~\ref{thm:window} is exactly
$\Gamma$.}
\label{tab:flatness}
\end{table}
...
$k=20$ (both give $\lm=22808$, $r=4344$). Random sequences are drawn without
...
\subsection{$\lm/r$ converges to $\Gamma$}

Put $Q=(\lm/r)/\Gamma$. Theorem~\ref{thm:sandwich} predicts
...
\midrule
8 & 1.096 & 0.492 & 1.904 (0.275) & $-4.909$ & 0.329 & 0.841\\
...
\caption{Convergence of $Q=(\lm/r)/W_D$. Error bars are one standard
...
The dispersion contracts steadily. Correlation between $\Gamma$ and $\lm/r$
...
predictors. This section is empirical throughout: it compares two statistics on
a fixed random model and claims no theorem. What it is meant to settle is a
question of \emph{which} quantity the theorems of \S\ref{sec:window} and
\S\ref{sec:sandwich} are about, not how well either quantity predicts.
...
\midrule
8 & $-0.874$ & $+0.841$ & $-0.926$ & $+0.176$ & $-0.466$\\
...
For $\lm/r$ --- the quantity our theorems actually control --- the separation
is sharper still: at $k=20$,
$\mathrm{partial}(\Gamma,\lm/r\mid CV)=+0.9986$
($p=2.4\times10^{-126}$) while
$\mathrm{partial}(CV,\lm/r\mid\Gamma)=+0.306$.
...
Correlations on a random model can only ever be suggestive, since the two
statistics are themselves correlated there ($\approx-0.85$). The clean
separation is to hold one fixed and vary the other. Both are invariants of the
set, so this is a question about which \emph{features} of the set each one
reads: $CV$ is a global second moment, while by Proposition~\ref{prop:gapform}
$\Gamma$ is a dyadically weighted sum of the gaps, dominated by the bottom of
the set. The two are therefore decouplable, and Table~\ref{tab:twins} decouples
them.
...
\toprule
$k$ & pair & $CV$ ratio & $\Gamma$ ratio & $\lm/r$ ratio\\
\midrule
12 & $CV$-twins & $1.000000$ & $2.135$ & $2.051$\\
13 & $CV$-twins & $1.000000$ & $2.070$ & $2.181$\\
14 & $CV$-twins & $1.000000$ & $2.240$ & $2.325$\\
\midrule
12 & $\Gamma$-twins & $1.628$ & $1.000000$ & $1.080$\\
13 & $\Gamma$-twins & $1.594$ & $1.000000$ & $1.034$\\
14 & $\Gamma$-twins & $1.600$ & $1.000000$ & $1.047$\\
\bottomrule
\end{tabular}
\caption{Holding one statistic fixed. A \emph{$CV$-twin pair} is two sets of $k$
odd numbers with the same cardinality, sum and sum of squares --- hence exactly
the same mean, variance and $CV$; a \emph{$\Gamma$-twin pair} is two sets with
exactly the same gap series. Each $\lm/r$ is a median over the $41$ targets
$n$ nearest $\sigma(A)/2$, computed by full enumeration of all $2^{k}$
configurations. When $CV$ is pinned, the landscape moves with $\Gamma$; when
$\Gamma$ is pinned, a $60\%$ change in $CV$ moves the landscape by at most
$8\%$.}
\label{tab:twins}
\end{table}

The pairs are exact, not approximate: $CV$-twins agree in $CV$ to the last bit
because they agree in $(k,\sum a_i,\sum a_i^2)$ as integers, and $\Gamma$-twins
agree in $\Gamma$ as rationals. For the $k=14$ $CV$-twins, for instance,
$\{1,11,31,\dots,103\}$ and $\{29,31,33,\dots,119\}$ both have
$CV=0.473014\ldots$, while $\Gamma=14.135$ and $31.658$ and the measured
$\lm/r$ are $13.60$ and $31.63$ --- each within $4\%$ of its own $\Gamma$,
and a factor $2.3$ apart, on data about which $CV$ says the two sets are
identical. The $\Gamma$-twin rows are the control that could have failed and
did not.

This is what Theorem~\ref{thm:window} predicts, and it is the sense in which the
correlation of \cite{AR14} is real but indirect: $CV$ tracks $\Gamma$ across a
random model, and only across it.
...
Theorem~\ref{thm:sandwich} yields $\lm_A(n)/r_A(n)\to\Gamma(A)$.
\end{problem}
...
\begin{remark}[There is no window analogue of Theorem~\ref{thm:floor}]
\label{rem:no-window-floor}
It is natural to ask whether Theorem~\ref{thm:floor} transfers from the
aggregate counts $R_c$ to the bounded windows $\mathrm{Win}_d$, giving a lower
bound $\varepsilon_d\gg2^{-|B_d|/2}$ valid for every odd sequence. It does not:
the window flatness can vanish identically. For $d=2$ and a sequence $A$ with
$\min A>4$ one has $I_2=\varnothing$, so $\mathrm{Win}_2=\{n-2,n,n+2\}$ has only
three points, and $r_{B_2}$ can be made constant on them. For example, with
\[
  A=(7,15,21,45,49,55,67,69,73,75,81,85,89,93,97,105,107,113,115,131,135,141,145,147,151,155,159,161)
\]
$(k=28)$ all three values equal $377736$, so $\varepsilon_2=0$ exactly. The
agreement is to one part in $3.8\times10^{5}$ and is therefore not an artefact
of the integrality of $r$; sequences with $\varepsilon_2=0$ were found by local
search at every size tested, with the common value growing from $10$ at $k=12$
to $377736$ at $k=28$.

The reason is simply that $\mathrm{Win}_d$ imposes $O(1)$ constraints on a
sequence with $k$ degrees of freedom. Theorem~\ref{thm:floor} constrains the
\emph{global} distribution of $r_B$ modulo $4$ and says nothing about any fixed
finite set of points. What survives is the statement for the sequences we care
about: for random odd sequences $\varepsilon_2$ sits at the modulus-$4$ scale
(\S\ref{sec:floor}), and for the primes it sits two orders of magnitude above
it --- but neither is forced by Theorem~\ref{thm:floor} alone.
\end{remark}
...
\[
  \frac{\lm_A(n)}{r_A(n)}-W_D(A)\;\approx\;
...
Every number in this paper is produced by a script that writes a log beside
itself. In particular \texttt{extremal\_r120} recomputes the constants of
\S\ref{sec:extremal} as exact rationals and re-tests
Corollary~\ref{cor:monotone} on $20{,}000$ random sets;
\texttt{cvtwin\_r120} constructs the twin pairs of Table~\ref{tab:twins} and
measures their landscapes by full enumeration; and \texttt{resweep\_r120}
regenerates the $100$-seed instances of \S\ref{sec:experiments} from the
recorded seeds and recomputes Tables~\ref{tab:Q} and~\ref{tab:cv}, reproducing
the recorded per-instance values to $5\cdot10^{-7}$ before doing so. The scripts, the logs, the Lean
development and the checks that enforce all of this are public at
\url{https://github.com/mathlab0911/arithmetic-landscapes}.
