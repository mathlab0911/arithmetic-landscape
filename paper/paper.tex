...
\begin{abstract}
For a finite set $A$ of odd positive integers with largest element $M$ we
introduce the \emph{gap series}
$\Gamma(A)=1+2\sum_{d=1}^{(M-1)/2}2^{-N_A(d)}\in\mathbb{Q}$, where
$N_A(d)=\#\{a\in A:a\le2d\}$; equivalently, $\Gamma$ is a dyadically weighted
sum of the gaps of $A$, dominated by the smallest elements. We show that
$\Gamma$ governs the combinatorics of subset sums of $A$.
...
$\lm/\deg$ to the flatness of truncated representation counts, are new. That
...
have $\lm/\deg$ a factor $2.1$ apart, while pairs with exactly equal $\Gamma$
and $CV$ a factor $1.6$ apart have $\lm/\deg$ within $8\%$
...
\toprule
$k$ & pair & $CV$ ratio & $\Gamma$ ratio & $\lm/\deg$ ratio\\
\midrule
...
exactly the same gap series. Each $\lm/\deg$ is a median over the $41$ targets
...
$\lm/\deg$ are $13.60$ and $31.63$ --- each within $4\%$ of its own $\Gamma$,
