...
equidistributed modulo $4$, with relative spread at least
$2\sqrt2\cdot2^{-|B|/2}$, a bound that is attained: no odd sequence is flat
beyond the scale $2^{-|B|/2}$. We also determine which congruence is
...
are never all equal, and
\[
  \frac{\max_c R_c-\min_c R_c}{\tfrac14\sum_c R_c}\;\ge\;2\sqrt2\cdot2^{-|B|/2},
\]
with equality attained. No odd sequence is flat beyond the scale $2^{-|B|/2}$.
\end{theoremE}
...
$3\in B$ then $F_B(\zeta_6)=0$. The primes are therefore not the flattest odd
sequences but among the least flat: their obstruction decays at rate
$\sqrt3/2\approx0.866$, above the universal floor $1/\sqrt2\approx0.707$.
...
Let $B$ be any finite sequence of odd positive integers. Then
\begin{enumerate}
\item $|F_B(i)|=2^{b/2}$, exactly, independently of the residues of the elements
      of $B$; in particular $F_B(i)\ne0$.
\item Writing $F_B(i)=2^{b/2}e^{i\varphi}$,
      \[
        \max_c R_c-\min_c R_c\;=\;2^{b/2}\max\bigl(|\cos\varphi|,|\sin\varphi|\bigr)
        \;\ge\;2^{(b-1)/2}.
      \]
\item Since $\sum_c R_c=2^{b}$, the relative spread satisfies
      \[
        \frac{\max_c R_c-\min_c R_c}{\frac14\sum_c R_c}\;\ge\;2\sqrt2\cdot2^{-b/2},
      \]
      and the constant $2\sqrt2$ is attained.
\end{enumerate}
\textup{(Lean: \Lean{normSq\_one\_add\_I\_pow\_odd}, \Lean{spread\_lower\_bound}.)}
\end{theorem}

\begin{proof}
(i) For odd $a$ we have $i^{a}=\pm i$, so $1+i^{a}=1\pm i$ and
$|1+i^{a}|=\sqrt2$ regardless of $a\bmod4$. Multiplying over $B$ gives
$|F_B(i)|=2^{b/2}$.
...
being odd. As $c$ runs over $\mathbb{Z}/4$ the quantity
$\mathrm{Re}(i^{-c}F_B(i))/2^{b/2}$ runs over
$\{\cos\varphi,\sin\varphi,-\cos\varphi,-\sin\varphi\}$, whose maximum is
$\max(|\cos\varphi|,|\sin\varphi|)\ge1/\sqrt2$ and whose minimum is its
negative. The stated identity and bound follow.

(iii) Divide by $\tfrac14\sum_cR_c=2^{b-2}$. Equality in (ii) occurs when
$\varphi$ is an odd multiple of $\pi/4$; for instance $B=(1,3,5,\dots,29)$
realises it.
\end{proof}
...
$a$. Hence flatness beyond the scale $2^{-b/2}$ is impossible for any odd
sequence, whereas the modulus-$6$ obstruction of the primes, decaying at rate
$\sqrt3/2\approx0.866$, sits \emph{above} the universal floor
$1/\sqrt2\approx0.707$. In this sense the primes are not the flattest odd
sequences but among the least flat, and $\varepsilon_k\to0$ --- if true --- can
hold at best geometrically, never faster than $2^{-k/2}$.
