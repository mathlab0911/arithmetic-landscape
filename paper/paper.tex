...
statistical-mechanics literature by a finite, checkable hypothesis. We compute
$\varepsilon$ exactly for $k\le24$: for the odd primes it decays geometrically
at rate $\approx\sqrt3/2$, matching to within $1\%$ a prediction derived from
the bias of primes modulo $6$, while for random odd sequences it decays
strictly faster. Over $100$ random sequences per $k\in\{8,12,16,18,20\}$ we
find $(\lm/\deg)/\Gamma\to1$ with standard deviation $0.0067$ at $k=20$ and
...
=\tfrac13\sum a^{2}$. Extracting the period-$6$ Fourier component of
$r_{B_d}/\mathrm{Main}-1$ numerically for $k\le28$ confirms both parts: the
amplitude ratio to $2(\sqrt3/2)^{|B_d|+1}$ is
$1.016,\,1.004,\,0.995,\,0.991,\,0.989$ at $k=20,\dots,28$, against
$0.880,\dots,0.856$ for the exponent $|B_d|$, which drifts; and the measured
phase agrees with $(\pi/6)(c_1-c_5)$ to four decimal places, including at
$k=16$, where $c_1-c_5=-3$ moves the predicted phase to $-\pi/2$.

\begin{conjecture}[Extremality of the modulus $6$]\label{conj:mod6}
Let $M(q)$ denote the geometric mean of $|\cos(\pi r/q)|$ over $r$ coprime to
$q$. Then $M(q)=\tfrac12|\Phi_q(-1)|^{1/\varphi(q)}$, where $\Phi_q$ is the
$q$-th cyclotomic polynomial, and $\sup_{q\ge3}M(q)=\sqrt3/2$, attained only at
$q=6$; the next largest value is $M(10)=5^{1/4}/2$. For a sequence
equidistributed among the residues coprime to $q$, $M(q)$ is the decay rate of
$|F_B(\zeta_q)|/2^{|B|}$, so $q=6$ is the slowest mode.
\end{conjecture}

The identity follows from $\prod_{r\perp q}(x-\zeta_q^{r})=\Phi_q(x)$ at $x=-1$
together with $|1+\zeta_q^{r}|=2|\cos(\pi r/q)|$, and the supremum from the
classical values $\Phi_q(-1)=p$ for $q=2p^{j}$, $2$ for $q=2^{j}$ with $j\ge2$,
and $1$ otherwise; we have checked the identity numerically for $3\le q\le40$ to
machine precision, obtaining $M(6)=0.866025$, $M(10)=0.747674$,
$M(4)=0.707107$, $M(14)=0.691544$. This is the precise sense in which the bias
of the primes modulo $6$, and no other congruence obstruction, governs the
flatness.
