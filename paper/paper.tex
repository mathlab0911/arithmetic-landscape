...
The combinatorial theorems below --- the classification, the window identity,
the stratification, the sandwich and the modulus-$4$ obstruction --- are
formally verified in Lean~4 with Mathlib; no proof contains \Lean{sorry} and no
additional axiom is used. One statement is verified only in part: for the
extremality of the modulus $6$ the finite check is formalised, while the
supremum over all $q$ rests on the classical values of $\Phi_q(-1)$. The
