...
For the sequence of odd primes, the relative amplitude of the period-$6$ ripple
in $r_{B_d}$ is $\asymp(\sqrt3/2)^{|B_d|}$, and consequently
$\varepsilon_d(k)\asymp(\sqrt3/2)^{k}$ for fixed $d$.
\end{conjecture}

The heuristic of \S\ref{sec:flatness-measured}(i) predicts the constant
$\sqrt3/2$, and the measurements of Table~\ref{tab:rates} agree to within
$1.1\%$ with $R^2\ge0.98$. A proof would require evaluating the relevant
character product exactly, and would give Problem~\ref{prob:flatness} for the
primes with a geometric rate.
...
$n=\lfloor T/2\rfloor$, and its sign is determined by $T\bmod6$.
\end{conjecture}
