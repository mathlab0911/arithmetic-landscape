...
zero-ineffective-constant rigidity theorem. The modulus-$4$ floor of papers 1--2, which is
universal for odd sequences, is then the exact leading ripple.}

\section{Two questions inherited from paper 3}\label{sec:inherited}
\TODO{(i) the $c_d$ fold-in: paper 3 keeps $\Phi$ as printed and counts
$c_d=e^{-\delta_d^{2}/2K''_d}$ in $E_k$, on the measurement that folding it in accounts for
at most $3\%$ of the residual. Extend that measurement across profiles and $k$ and decide.
(ii) the mechanism behind $c_A=c_A'$, conjectured in paper 3's \textup{rem:onek}.}
