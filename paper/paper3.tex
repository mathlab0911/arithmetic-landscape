...
\TODO{fable: the bookkeeping of the constant $C$ against P3's local limit theorem is the one
place this could still pinch. $\theta_1\asymp\sqrt{b/V}$ is $\sqrt b$ times the natural LCLT
...
Among $q\ge3$ the quantity $\mu(q)/\varphi(q)$ of Lemma~\ref{lem:eta}(b) is maximal at
$q=6$, where it equals $+\tfrac12$, and \textbf{the maximum is attained there only}: $\varphi(q)=2$
forces $q\in\{3,4,6\}$, and $\mu(3)=-1$, $\mu(4)=0$, $\mu(6)=+1$. So the worst point of the
additive estimate is $\theta=1/6$, with $\operatorname{Re}S_A/b\to\tfrac12=\cos60^\circ$.

Paper~2's multiplicative estimate has its worst point at the same $q$: $\sup_qM(q)=M(6)=
\sqrt3/2=\cos30^\circ$. Both are the statement that the odd primes lie in $\pm1\pmod 6$ and
nowhere else, read once through $\prod|\cos|$ and once through $\sum\cos$. Measured on the odd
primes below $10^{7}$, $\operatorname{Re}S_A(1/6)/b=0.499998$ against the predicted $1/2$, and
$q=6$ is the maximiser over $2\le q\le16$.
