...
\STATUS{(a) elementary, sketched; (b) proved, Lemma~\ref{lem:etab}; (c) quoted from paper 2}
...
The Siegel--Walfisz step is the only inexplicit estimate in the whole of \S\ref{sec:bridge}:
the constant $c$ above is the one that cannot be computed. It is the same constant that
paper~2's appendix isolates as $C_1$, entering that paper at one point as well; see
Remark~\ref{rem:oneconstant}. Every other bound here --- Lemma~\ref{lem:kappa},
Lemma~\ref{lem:eta}(a), Lemma~\ref{lem:six}, and the quotation in
Lemma~\ref{lem:eta}(c), which rests on Vaughan's identity --- is effective.
\end{remark}
