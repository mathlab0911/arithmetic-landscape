...
\paragraph{Where the object comes from.}
$F_k$ arose as a one-parameter deformation of an arithmetic invariant, in which $w_j$ is
a normalised gap statistic of a finite set of odd integers \cite{P1,P3}. \emph{Nothing
below uses that}, and the reader who stops at this paragraph loses nothing.

\subsection{Terminology}\label{sec:terminology-note}

\emph{This note deliberately carries no vocabulary of its own.} Everything below is
stated in the standard language of power series and their sections. Two phrases appear
only in the titles of the companion papers cited at the end, and are glossed here so
that a reader who meets them is not sent away:
...
\midrule
\emph{gap series} & the arithmetic invariant of \cite{P1} from which the weights $w_j$
  of this note arise as a normalised gap statistic. Not used here.\\
\emph{coset identity} & an exact averaging identity of \cite{P3} for
  $-\log|\cos\pi t|$ over an arithmetic progression, unrelated to the present
  question and named only because it appears in that paper's title. Not used here.\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{corollary}[the pinch at $R=1$, for constant weights]\label{cor:pinch}
\STATUS{proved; immediate from Theorem~\ref{thm:const}(d).}
If $w_j\to w>0$ with $w_j$ eventually constant, then $R=1$ and the zeros of $F_k$
approach $q=\tfrac12$ along the line $\operatorname{Re}q=\tfrac12$ at rate $\pi/2k$.
\end{corollary}
...
$\lambda_{\mathrm{eff}}\to s-\tfrac12$ & $|\lambda_{\mathrm{eff}}-(s-\tfrac12)|$ stays
  in $[0.34,0.48]$ and does not shrink & \textbf{refuted}\\
...
\begin{proposition}[the naive balance is refuted]\label{prop:notshalf}
\STATUS{measured and decisive, \texttt{lambda\_r206}: $|\lambda_{\mathrm{eff}}-(s-\tfrac12)|$
stays in $[0.34,0.48]$ over $k\le2048$ at five exponents and does not shrink.}
For $w_j=(j+1)^{-s}$, $\lambda_{\mathrm{eff}}(k,s)$ does not approach $s-\tfrac12$.
\end{proposition}

So the extra factor $1/\theta$ is not present, and \emph{a correct treatment must explain
why the oscillatory sum does not gain it.} We believe the reason is that the amplitude
$w_j\rho^{\,j}$ is not slowly varying on the scale of one half-period near $j=k$, but we
have not turned that into an estimate, and we are not stating it as a claim.
...
(Remark~\ref{rem:whyline}). Each is corrected at its statement in the companion paper and
named in the release notes there.
...
\bibitem{P1} K.~Amauchi, \emph{Arithmetic landscapes I: the gap series}.
  \url{https://github.com/mathlab0911/arithmetic-landscapes}
\bibitem{P3} K.~Amauchi, \emph{Arithmetic landscapes III: deformed measures, random
  sequences, and the coset identity}.
  \url{https://github.com/mathlab0911/arithmetic-landscapes}
\end{thebibliography}
