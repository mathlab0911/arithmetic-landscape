...
| `paper/paper.tex` | **The paper** (LaTeX, arXiv-ready). `make` builds `paper.pdf` |
| `paper/fig_*.pdf` | Figures, generated by `lean/pnp/figures_r9.py` |
| `paper1_draft.md` | Markdown draft the LaTeX was derived from (retains Japanese notes) |
...
```
cd paper
make          # -> paper.pdf   (needs latexmk + pdflatex)
make figs     # regenerate the figures from the experimental data
```

Verified to build with TeX Live 2021: 15 pages, no undefined references.
