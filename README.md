...
*Archived release `v1.3.0` (2026-08-18). The DOI above resolves to the latest version; each release also gets its own. Manuscripts CC BY 4.0, code Apache-2.0. **Not peer-reviewed** — every statement carries its own status where it is stated.*
...
`v1.3.0` **adds rather than corrects**: two proved theorems in the standalone note, and with them the written proof of the second one as a public artefact (`paper/proof_decayrate.md`). It also records how close that release came to going out with nine bad sentences. The referee pass this project runs before a milestone — a reader in fresh context, given three jobs and nothing else — read the nine units of the new section and **flagged all nine**, on text that was green on all twenty-two checks and built in both languages with zero undefined references. One of them was false as written; one claimed *proved* for a theorem whose proof `.gitignore` was excluding from the repository. The pass and its adjudication are `lean/pnp/refpass_r233.log`.
...
itself — **195 scripts, 275 logs**, all committed. A number with no log is treated as a number
