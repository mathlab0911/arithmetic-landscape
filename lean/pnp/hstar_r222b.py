#!/usr/bin/env python3
# hstar_r222b.py -- the SAME derivation, one term further, and criteria that use the
...
def C_of(s):
    """C_s = 2^s Gamma(1-s) sin(pi s/2), COMPUTED (F87)."""
    return float(mpf(2) ** mpf(s) * gamma(1 - mpf(s)) * mp.sin(mp.pi * mpf(s) / 2))
