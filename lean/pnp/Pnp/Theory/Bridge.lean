/- 算術地形理論(ALT)正典 第3号 (2026-08-07, opus-5 実務セッション・4周目)
   3周目の持ち越し:
     ...
     定理17〜19  窓級数 = gapSeries 恒等式(増加奇数列)
   これで「実験のビットマスク列挙」「gs(List版)」「energy(Finset版)」が
   完全に同一の対象であることが形式的に確定し、Γ が局所最小の窓の総和として
   純算術的に特徴づけられる。 -/
...
/- ============ 定理17〜18: 窓級数 = gapSeries 恒等式 ============
   ...
   が「局所最小数 / 基底縮退度」の予測値になる(実験値 lm/deg に対応)。
   本節はこの W が Γ = gapSeries と厳密に一致することを示す。 -/
