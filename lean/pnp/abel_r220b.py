...
#       at 60 digits, over a grid of (k, s, t).
#
#       *** THIS CRITERION IS A REPLACEMENT, AND THE REPLACEMENT IS ASTERISKED. ***
#       abel_r220 registered "agreement to 1e-45 ABSOLUTE" and recorded FAIL.  The
#       identity was reproduced to 60 significant digits at every point; what failed was
#       the criterion, which put an absolute tolerance on a quantity that the same script
#       evaluates from 1e0 to 1e62.  At t = 2.7 t_pred, F ~ 1.3e62 and the two routes
#       differed by 3168 -- a relative 2.4e-59.
#
#       The defect is visible WITHOUT the data (a tolerance of 1e-45 on a number the
#       script itself prints as 1e62), which is why this is repaired rather than
#       abandoned.  But it is still a criterion rewritten after a run, so: the original
#       log `abel_r220.log` is kept, committed, and cited here.  A pre-registration
#       edited after seeing data is not a pre-registration (r206), and the way to keep
#       that true is to leave the first one standing.
#
#       Replacement, scaled to what is being tested at each point:
#         A1a  RELATIVE agreement < 1e-50 at every point.  (The generic check.)
#         A1b  ABSOLUTE agreement < 1e-45 AT THE ZERO ITSELF, where F = 0 and the
#              cancellation is total -- an absolute tolerance is the correct one exactly
#              there, and only there.  This is the informative half: at t = 2.7 t_pred the
#              sum is dominated by one term and agreement is nearly free.
#
...
# ------------------------------------------------------------------ A1
say()
say("--- A1a  THE IDENTITY, RELATIVE agreement (see the asterisk in the header) ---")
say("  %5s %6s %14s %24s %12s" % ("s", "k", "t", "F (both routes)", "rel diff"))
okA1, nA1 = True, 0
for ss in ('1', '2', '4'):
    for k in (64, 256, 1024):
        ...
        for lab, t in (('0.3*pred', pred * mpf('0.3')),
            ...
            rel = abs(a - b) / max(abs(a), mpf(1))
            nA1 += 1
            if rel > mpf('1e-50'):
                okA1 = False
            say("  %5s %6d %14s %24s %12s"
                % (ss, k, lab, mp.nstr(a, 18), mp.nstr(rel, 4)))
say("  cases: %d -> %s" % (nA1, "PASS" if okA1 and nA1 else "FAIL"))
verdicts.append(('A1a identity, relative', okA1 and nA1 > 0))

say()
say("--- A1b  THE INFORMATIVE HALF: absolute agreement AT THE ZERO, where F = 0 ---")
say("  there the direct sum cancels completely, so an absolute tolerance is the right one")
say("  %5s %6s %20s %20s %14s" % ("s", "k", "t_1", "F_direct(t_1)", "|direct-abel|"))
okA1b, nA1b = True, 0
ZEROS = {('1', 64): '0.186199681451984', ('1', 256): '0.106767212545108',
         ('1', 1024): '0.0543428673092', ('2', 256): '0.1472309802',
         ('4', 1024): '0.1173883585'}
for (ss, k), tv in sorted(ZEROS.items(), key=lambda x: (x[0][0], x[0][1])):
    w = w_power(k, ss)
    t = mpf(tv)
    a, b = F_direct(w, t), F_abel(w, t)
    d = abs(a - b)
    nA1b += 1
    if d > mpf('1e-45'):
        okA1b = False
    say("  %5s %6d %20s %20s %14s"
        % (ss, k, tv, mp.nstr(a, 6), mp.nstr(d, 4)))
say("  cases: %d -> %s" % (nA1b, "PASS" if okA1b and nA1b else "FAIL"))
verdicts.append(('A1b identity at the zero, absolute', okA1b and nA1b > 0))
okA1 = okA1 and okA1b
